...
extern "C" __global__ void conv2d_multi(float *out, float *x, float *kernels, int N, int M, int L, int K, int k_out, int stride) {
...
extern "C" __global__ void mean_pooling_multi(float *out, float *x, int N, int M, int L, int K, int stride) {
    int radius = K / 2;
    for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < (int)ceilf((float)N / stride) - radius; i += blockDim.x * gridDim.x) {
        int out_index = M * i / stride;
        int i_f = i * stride + radius;
...
extern "C" __global__ void gap_multi(float *out, float *x, int N, int M, int L) {
...
extern "C" __global__ void dot_product_multi(const float *x, const float *y, float *z, int N) {
    int warp_size = 32;
    float sum = float(0);
    for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < N; i += blockDim.x * gridDim.x) {
        sum += x[i] * y[i];
...
extern "C" __global__ void concat_multi(float *z, const float *x, const float *y, int n) {
    for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < n; i += blockDim.x * gridDim.x) {
        z[i] = x[i];
        z[i + n] = y[i];
    }
...
void Benchmark20::execute_sync(int iter) {
    ...
    conv2d_multi<<<grid_size_2, block_size_2d_dim, K * K * kn1 * channels * sizeof(float)>>>(x1, x, kernel_1, N, N, channels, K, kn1, stride);
    cudaDeviceSynchronize();
    conv2d_multi<<<grid_size_2, block_size_2d_dim, K * K * kn1 * channels * sizeof(float)>>>(y1, y, kernel_3, N, N, channels, K, kn1, stride);
    cudaDeviceSynchronize();

    mean_pooling_multi<<<grid_size_3, block_size_3d_dim>>>(x11, x1, N / stride, N / stride, kn1, pooling_diameter, pooling_diameter);
    cudaDeviceSynchronize();
    mean_pooling_multi<<<grid_size_3, block_size_3d_dim>>>(y11, y1, N / stride, N / stride, kn1, pooling_diameter, pooling_diameter);
    cudaDeviceSynchronize();

    conv2d_multi<<<grid_size_2, block_size_2d_dim, K * K * kn1 * kn2 * sizeof(float)>>>(x2, x11, kernel_2, N / stride / pooling_diameter, N / stride / pooling_diameter, kn1, K, kn2, stride);
    cudaDeviceSynchronize();
    conv2d_multi<<<grid_size_2, block_size_2d_dim, K * K * kn1 * kn2 * sizeof(float)>>>(y2, y11, kernel_4, N / stride / pooling_diameter, N / stride / pooling_diameter, kn1, K, kn2, stride);
    cudaDeviceSynchronize();
    ...
    concat_multi<<<num_blocks, block_size_1d>>>(z, x2, y2, x2_len);
    cudaDeviceSynchronize();

    dot_product_multi<<<num_blocks, block_size_1d>>>(z, dense_weights, res, x2_len);
    cudaDeviceSynchronize();
}

void Benchmark20::execute_async(int iter) {
    cudaSetDevice(0);            // Set device 0 as current
    ...
    cudaSetDevice(0);            // Set device 0 as current
    conv2d_multi<<<grid_size_2, block_size_2d_dim, K * K * kn1 * channels * sizeof(float), s1>>>(x1, x, kernel_1, N, N, channels, K, kn1, stride);
    mean_pooling_multi<<<grid_size_3, block_size_3d_dim, 0, s1>>>(x11, x1, N / stride, N / stride, kn1, pooling_diameter, pooling_diameter);
    conv2d_multi<<<grid_size_2, block_size_2d_dim, K * K * kn1 * kn2 * sizeof(float), s1>>>(x2, x11, kernel_2, N / stride / pooling_diameter, N / stride / pooling_diameter, kn1, K, kn2, stride);

    cudaSetDevice(1);            // Set device 1 as current
    conv2d_multi<<<grid_size_2, block_size_2d_dim, K * K * kn1 * channels * sizeof(float), s2>>>(y1, y, kernel_3, N, N, channels, K, kn1, stride);
    mean_pooling_multi<<<grid_size_3, block_size_3d_dim, 0, s2>>>(y11, y1, N / stride, N / stride, kn1, pooling_diameter, pooling_diameter);
    conv2d_multi<<<grid_size_2, block_size_2d_dim, K * K * kn1 * kn2 * sizeof(float), s2>>>(y2, y11, kernel_4, N / stride / pooling_diameter, N / stride / pooling_diameter, kn1, K, kn2, stride);
    ...
    concat_multi<<<num_blocks, block_size_1d, 0, s1>>>(z, x2, y2, x2_len);

    dot_product_multi<<<num_blocks, block_size_1d, 0, s1>>>(z, dense_weights, res, x2_len);
